...
const Navbar = ({ isBelow480px, isBelow768px, isLoggedIn }: any) => {
  const {
    dispatch,
    pathsArr,
    reposList,
  ...
  return (
    <Box css={css.Outer}>
      <Box css={css.TopNavBox}>
        ...
        >
          <Image src={Logo} />
          <Box ml={4}> {/* Add margin-left to create space */}
          <Text fontFamily="Funnel Display" fontSize="35px" fontWeight="">CodeAIx</Text>
            {isBelow480px && <Text css={css.SecondTitle}>AI Code Editor</Text>}
          </Box>
        </Box>
        {!isBelow480px && <Text css={css.SecondTitle} ml={4}></Text>} {/* Add margin-left to create space */}
        <Box display="flex" alignItems="center" gap={2} ml="auto">
  {isLoggedIn ? (
    <Button as={Link} to="/account" css={css.ImportBtn}>
      <Text>Account</Text>
    </Button>
  ) : (
    <>
      <Button as={Link} to="/signup" css={css.ImportBtn}>
        <Text>Sign Up</Text>
      </Button>
      <Button as={Link} to="/login" css={css.ImportBtn}>
        <Text>Login</Text>
      </Button>
    </>
  )}
